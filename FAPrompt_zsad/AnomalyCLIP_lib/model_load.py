...
__all__ = ["available_models", "load", 
           "get_similarity_map",  "compute_similarity", "compute_similarity_ori"]
_tokenizer = _Tokenizer()

_MODELS = {
    "ViT-L/14@336px": "https://openaipublic.azureedge.net/clip/models/3035c92b350959924f9f00213499208652fc7ea050643e8b385c2dac08641f02/ViT-L-14-336px.pt",
    "ViT-B-16-plus-240": "https://github.com/mlfoundations/open_clip/releases/download/v0.2-weights/vit_b_16_plus_240-laion400m_e32-699c4b84.pt"
}
...
def _download(
    ...
    if not cache_dir:
        ...
        cache_dir = os.path.expanduser("/home/zhujiawen/AnomalyCLIP/.cache/clip")
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", design_details = None, jit: bool = False, download_root: str = None):
    ...
    if name in _MODELS:
        ...
        # model_path = _download(_MODELS[name], download_root or os.path.expanduser("/home/zhujiawen/AnomalyCLIP/.cache/clip"))
...
def get_similarity_map(sm, shape):
    ...
    sm = sm.reshape(side, side, sm.shape[1], sm.shape[2]).permute(2, 3, 0, 1)
    sm = torch.nn.functional.interpolate(sm, shape, mode='bilinear')
    return sm

def compute_similarity_ori(image_features, text_features, t=2):
    prob_1 = image_features[:, :1, :] @ text_features.t()  # 1370*1*768 2 768
    b, n_t, n_i, c = image_features.shape[0], text_features.shape[0], image_features.shape[1], image_features.shape[2]
    feats = image_features.reshape(b, n_i, 1, c) * text_features.reshape(1, 1, n_t, c)  # [1370, 8, 1, 768]  [1, 1, 2, 768]
...
def compute_similarity(image_features, text_features, t=2):
    ...
    feats = image_features.reshape(b, n_i, 1, c) * text_features.reshape(1, n_i, n_t, c)
    similarity = feats.sum(-1)
    return (similarity/0.07).softmax(-1), prob_1
