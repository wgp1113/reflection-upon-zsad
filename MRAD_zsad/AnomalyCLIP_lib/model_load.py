...
def compute_similarity(image_features, text_features, sign =False):#cr_modify
    if sign:
        ...
        similarity = torch.matmul(image_features, text_features.transpose(1, 2))  # [8, 1370, 2]
#     b, n_i, c = image_features.shape
#     b_t, n_t, c_t = text_features.shape


#     # 2. 归一化特征
#     image_features = image_features / image_features.norm(dim=-1, keepdim=True)  # [b, n_i, c]
#     text_features = text_features / text_features.norm(dim=-1, keepdim=True)  # [b, n_t, c]

#     # 3. 计算余弦相似度
#     similarity = torch.bmm(image_features, text_features.transpose(1, 2))  # [b, n_i, c] @ [b, c, n_t] -> [b, n_i, n_t]
