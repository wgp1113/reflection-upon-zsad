...
# from AnomalyCLIP_lib.lora import LoRA
...
class Attention(nn.Module):
    ...
    #     B, N, C = x.shape
    #     qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
    #     qkv_lora = self.lora(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
    #     qkv = qkv+qkv_lora
    #     q, k, v = qkv[0], qkv[1], qkv[2]
...
class VisionTransformer(nn.Module):
    ...
    def DAPM_replace(self, DPAM_layer):
        if DPAM_layer is not None:
            for i in range(1, DPAM_layer):
                ...
                # print(self.attn)
                # print(self.transformer.resblocks[-i].attn.parameters)
                # for name, param in self.transformer.resblocks[-i].named_parameters():
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        if True:
            patch_token_list = []
            patch_token_proj_list = []
            cls_token_list = []
            for patch_token in patch_tokens:
                ori_pat = patch_token
                patch_token_proj = self.ln_post(patch_token.permute(1, 0, 2)) @ self.proj  # LND -> NLD
                patch_token = self.ln_post(patch_token.permute(1, 0, 2))
                patch_token_list.append(patch_token[:,1:,:])
                cls_token_list.append(ori_pat[0, :, :]@ self.proj)
                patch_token_proj_list.append(patch_token_proj[:,1:,:])
            patch_tokens = patch_token_list
            cls_tokens = cls_token_list

            return x_ori[0, :, :]@ self.proj, patch_tokens, cls_tokens,patch_token_proj_list    
...
class AnomalyCLIP(nn.Module):
    ...
    def encode_text_learn(self, prompts, tokenized_prompts, deep_compound_prompts_text = None, normalize: bool = False):
        ...
        batch,num_prompts,max_len ,dim = prompts.shape
        prompts = prompts.reshape(batch*num_prompts,max_len,dim)
        ...
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        tokenized_prompts = tokenized_prompts.unsqueeze(0).expand(batch,-1,-1).reshape(batch*num_prompts,max_len)
        x = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection
        x = x.reshape(batch,num_prompts,x.shape[-1])
        return x
