...
def Split_Product(dataset_name):
	if dataset_name == "visa":
		...
		group1 = ["pipe_fryum", "cashew", "macaroni1"]
	...
	if dataset_name == "mvtec":
		...
		group1 = ["transistor", "capsule", "screw", "cable"]
		group2 = ["grid", "wood", "carpet", "toothbrush"]
		# group3 = ["bottle", "tile", "hazelnut", "pill", "zipper"]
		group3 = ["leather", "tile", "hazelnut", "zipper"]
...
class Makedataset():
	...
	def mask_dataset(self, name, product_list, batchsize, shuf = True):
		...
		if name == "mvtec" and self.train_mode == "zero":
			dataset = MVTecDataset(root=self.train_data_path, transform=self.preprocess_test, target_transform=self.target_transform,
								  aug_rate=self.aug_rate, mode =self.mode, train_mode=self.train_mode, product_list= product_list, dataset="mvtec_rethink_zsad")
		elif name == "visa" and self.train_mode == "zero":
			dataset = VisaDataset(root=self.train_data_path, transform=self.preprocess_test, target_transform=self.target_transform,
											   mode =self.mode, train_mode=self.train_mode, product_list= product_list, dataset= "visa_rethink_zsad")
		else:
			dataset = OtherDataset(root=self.train_data_path, transform=self.preprocess_test,
								   target_transform=self.target_transform,
								   aug_rate=self.aug_rate, mode=self.mode, train_mode=self.train_mode,
								   product_list=product_list, dataset=name)
...
class MVTecDataset(data.Dataset):
	...
	def combine_img(self, cls_name):
		...
		img_paths = os.path.join(self.root, "mvtec_rethink_zsad", cls_name, 'test')
		img_ls = []
		mask_ls = []
		for i in range(4):
			...
			mask_path = os.path.join(self.root, "mvtec_rethink_zsad",cls_name, 'ground_truth', random_defect, random_file.replace("bmp", "png"))
...
class OtherDataset(data.Dataset):
	def __init__(self, root, transform, target_transform, aug_rate, mode='train', train_mode='zero', aug=True,
				 dataset="mvtec", product_list=None):
		self.root = root
		self.transform = transform
		self.target_transform = target_transform
		self.aug_rate = aug_rate
	...
	def combine_img(self, cls_name):
		...
		for i in range(4):
			...
			random_file = random.choice(files)
			img_path = os.path.join(img_paths, random_defect, random_file)
			mask_path = os.path.join(self.root, cls_name, 'ground_truth', random_defect,
