...
class WFDD_dataset():
    def __init__(self,path_root):
        ...
        self.dataset_name = ['grey_cloth', 'grid_cloth', 'pink_flower', 'yellow_cloth']
            ...
            for mode in ["test"]:
                ...
                if len(defect_classes) == 1 and "good" in defect_classes and mode=="train":
                    ...
                    for i in range(len(data_list)):
                        ...
                        save_img_path = os.path.join(des_path_root,new_class_name,"train","good","wfdd_"+str(id).zfill(6)+".bmp")
                        ...
                        for classes in defect_classes:
                            ...
                            if classes == "good":
                                for i in range(len(data_list)):
                                    ...
                                    new_class_name = data_name
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","good","wfdd_"+"good_"+str(id).zfill(6)+".bmp")
                                    cv2.imwrite(save_img_path,raw_img)
                                    id = id + 1
                            else:
                                for i in range(len(data_list)):
                                    ...
                                    new_class_name = data_name
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","anomaly",f"wfdd_{classes}_"+str(id).zfill(6)+".bmp")
                                    save_mask_path = os.path.join(des_path_root,new_class_name,"ground_truth","anomaly",f"wfdd_{classes}_"+str(id).zfill(6)+".png")
