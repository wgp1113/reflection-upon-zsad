...
class VisA_zsad_dataset():
    ...
    def make_VAND(self,binary,to_255,des_path_root,id):
        ...
        for data_name in self.dataset_name:
            ...
            for mode in ["test"]:
                data_path = os.path.join(self.path_root,data_name, 'Data', 'Images')
                defect_classes = sorted(os.listdir(data_path))
                ...
                if len(defect_classes) > 1 and mode == "test":
                    for classes in defect_classes:
                        data_list = glob.glob(os.path.join(data_path,classes,'*.JPG'))
                        data_list = sorted(data_list)
                        if classes == "Normal":
                            for i in range(len(data_list)):
                                #name_new = data_name.capitalize()
                                ...
                                #raw_mask = cv2.imread(raw_mask_path,cv2.IMREAD_GRAYSCALE)
                                new_class_name = data_name
                                save_img_path = os.path.join(des_path_root,new_class_name,"test","good","visa_"+"good_"+str(id).zfill(6)+".bmp")
                            ...
                            for i in range(len(data_list)):
                                ...
                                raw_mask_path = os.path.join(self.path_root,data_name, 'Data','Masks',classes,os.path.basename(data_list[i]).replace(".JPG",".png"))
                                image_name_id = data_list[i].split('/')[-1].split('.')[0]
                                raw_img = cv2.imread(data_list[i],cv2.IMREAD_COLOR)
                                raw_mask = cv2.imread(raw_mask_path,cv2.IMREAD_GRAYSCALE)
                                if binary:
                                ...
                                new_class_name = data_name
                                save_img_path = os.path.join(des_path_root,new_class_name,"test","anomaly",f"visa_{classes}_"+image_name_id+'_'+str(id).zfill(6)+".bmp")
                                #save_img_path = os.path.join(des_path_root,new_class_name,"test","anomaly",f"mvtec_{classes}_"+str(id).zfill(6)+".png")
                                save_mask_path = os.path.join(des_path_root,new_class_name,"ground_truth","anomaly",f"visa_{classes}_"+image_name_id+'_'+ str(id).zfill(6)+".png")
