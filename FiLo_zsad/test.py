...
from datasets.other_dataset import OtherDataset
...
dtd_anomaly_detail_gpt = {
    "Blotchy 099": "uneven surface color, irregular dark patches, surface pitting or erosion, localized staining, mottled texture, hairline cracks, salt efflorescence, inconsistent mineral distribution, weathering marks, presence of foreign residues",
    "Fibrous 183": "broken fibers, clumps of tangled material, localized thinning of the fiber density, presence of large gaps or holes, irregular fiber orientation, scorched or burnt strands, foreign contaminants trapped within the web, discoloration or uneven staining, damp or moldy patches, shedding or loose fiber debris",
    "Marbled 078": "surface pitting, uneven mineral veins, localized discoloration, hairline cracks, surface erosion or weathering, irregular dark spots, salt efflorescence, inconsistent texture, staining from liquid absorption, presence of foreign residue",
    "Matted 069": "broken fibers, tangled clumps, localized thinning of material, presence of holes or gaps, irregular fiber density, scorched or burnt areas, foreign contaminants embedded in the mesh, uneven discoloration or staining, damp or moldy patches, shedding of loose fiber debris",
    "Mesh 114": "broken wire, deformed grid pattern, uneven wire spacing, localized oxidation or rust, metal fatigue cracks, crushed or flattened sections, foreign particles caught in the mesh, frayed wire ends, inconsistent strand thickness, signs of mechanical impact",
    "Perforated 037": "clogged or blocked holes, irregular hole shapes, cracks between perforations, surface scratches, dents or deformations in the metal sheet, rust or oxidation spots, missing punch holes, inconsistent spacing between elements, burrs around the hole edges, oily or chemical stains",
    "Stratified 154": "vertical cracks, irregular banding, localized discoloration, surface peeling, inconsistent layer thickness, warping of the grain, staining from water damage, presence of knots, rough patches, deep scratches along the texture",
    "Woven 001": "broken or snapped strands, irregular weaving pattern, thinning or bald spots in the weave, loose or protruding fibers, snags or pulled loops, contamination or foreign particles embedded in the mesh, frayed edges, discoloration or staining, compression or flattened areas, inconsistencies in strand thickness",
    "Woven 068": "broken or snapped strands, irregular weaving pattern, thinning or bald spots in the weave, loose or protruding fibers, snags or pulled loops, contamination or foreign particles embedded in the mesh, frayed edges, discoloration or staining, compression or flattened areas, inconsistencies in strand thickness",
    "Woven 104": "broken or snapped strands, irregular weaving pattern, thinning or bald spots in the weave, loose or protruding fibers, snags or pulled loops, contamination or foreign particles embedded in the mesh, frayed edges, discoloration or staining, compression or flattened areas, inconsistencies in strand thickness",
    "Woven 125": "broken or snapped strands, irregular weaving pattern, thinning or bald spots in the weave, loose or protruding fibers, snags or pulled loops, contamination or foreign particles embedded in the mesh, frayed edges, discoloration or staining, compression or flattened areas, inconsistencies in strand thickness",
    "Woven 127": "broken or snapped strands, irregular weaving pattern, thinning or bald spots in the weave, loose or protruding fibers, snags or pulled loops, contamination or foreign particles embedded in the mesh, frayed edges, discoloration or staining, compression or flattened areas, inconsistencies in strand thickness",
}

ksdd2_anomaly_detail_gpt = {
    "surface": "vertical cracks, irregular banding, localized discoloration, surface peeling, inconsistent layer thickness, warping of the grain, staining from liquid absorption, presence of tangled fibers, rough patches, deep scratches along the texture",
}

wfdd_anomaly_detail_gpt = {
    "grey cloth": "discoloration in a specific area, irregular patch or section with a different texture, frayed edges or unraveling fibers, burn mark or scorching, thinning or bald spots in the weave, loose or protruding fibers, snags or pulled loops, contamination or foreign particles embedded in the fabric, oily or chemical stains, deep scratches or tears",
    "grid cloth": "crooked weaving, cracks or tears in fabric, excessive gaps between threads, discoloration in a specific area, deformation of the grid pattern, missing threads, inconsistent spacing between grid elements, frayed edges or unraveling fibers, burn mark or scorching, contamination or foreign particles",
    "pink flower": "discoloration in a specific area, irregular patch or section with a different texture, frayed edges or unraveling fibers, burn mark or scorching, thinning or bald spots in the weave, loose or protruding fibers, snags or pulled loops, contamination or foreign particles embedded in the fabric, oily or chemical stains, misprinted or blurred flower pattern, missing print elements",
    "yellow cloth": "discoloration in a specific area, irregular patch or section with a different texture, frayed edges or unraveling fibers, burn mark or scorching, thinning or bald spots in the weave, loose or protruding fibers, snags or pulled loops, contamination or foreign particles embedded in the fabric, oily or chemical stains, deep scratches or tears",
}
...
for cls_name in wfdd_anomaly_detail_gpt.keys():
    wfdd_anomaly_detail_gpt[cls_name] = (
        wfdd_anomaly_detail_gpt[cls_name].split(",")
    )

for cls_name in ksdd2_anomaly_detail_gpt.keys():
    ksdd2_anomaly_detail_gpt[cls_name] = (
        ksdd2_anomaly_detail_gpt[cls_name].split(",")
    )

for cls_name in dtd_anomaly_detail_gpt.keys():
    dtd_anomaly_detail_gpt[cls_name] = (
        dtd_anomaly_detail_gpt[cls_name].split(",")
    )
...
for cls_name in dtd_anomaly_detail_gpt.keys():
    status_abnormal[cls_name] = ['abnormal {} ' + 'with {}'.format(x) for x in dtd_anomaly_detail_gpt[cls_name]]

for cls_name in ksdd2_anomaly_detail_gpt.keys():
    status_abnormal[cls_name] = ['abnormal {} ' + 'with {}'.format(x) for x in ksdd2_anomaly_detail_gpt[cls_name]]

for cls_name in wfdd_anomaly_detail_gpt.keys():
    status_abnormal[cls_name] = ['abnormal {} ' + 'with {}'.format(x) for x in wfdd_anomaly_detail_gpt[cls_name]]
...
def cal_score(obj):
    ...
    table.append(str(np.round(auroc_px * 100, decimals=4)))
    table.append(str(np.round(f1_px * 100, decimals=4)))
    table.append(str(np.round(ap_px * 100, decimals=4)))
    table.append(str(np.round(aupro * 100, decimals=4)))

    table.append(str(np.round(auroc_sp * 100, decimals=4)))
    table.append(str(np.round(f1_sp * 100, decimals=4)))
    table.append(str(np.round(ap_sp * 100, decimals=4)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_path", type=str, default="default", help="save_path"
    )
    ...
    for items in tqdm(test_dataloader):
        ...
        with torch.no_grad():
            ...
            if dataset_name == "mvtec":
                ...
                text_prompt = " . ".join(anomaly_status_general + dtd_anomaly_detail_gpt[cls_name])
            elif dataset_name == "wfdd":
                text_prompt = " . ".join(anomaly_status_general + wfdd_anomaly_detail_gpt[cls_name])
            elif dataset_name == "ksdd2":
                text_prompt = " . ".join(anomaly_status_general + ksdd2_anomaly_detail_gpt[cls_name])
            ...
            if dataset_name == "mvtec":
                ...
                for i in range(boxes_filt.size(0)):
                    if not check_elements_in_array(dtd_anomaly_detail_gpt[cls_name] + anomaly_status_general,
                                                   pred_phrases[i]):
                        pred_phrases[i] += "#$%"
                        # boxes_filt_copy = torch.cat([boxes_filt_copy[:i], boxes_filt_copy[i+1:]])
                        continue
                ...
                for i in range(boxes_filt.size(0)):
                    if not check_elements_in_array(ksdd2_anomaly_detail_gpt[cls_name] + anomaly_status_general,
                                                   pred_phrases[i]):
                        pred_phrases[i] += "#$%"
                        # boxes_filt_copy = torch.cat([boxes_filt_copy[:i], boxes_filt_copy[i+1:]])
                        continue
                ...
                for i in range(boxes_filt.size(0)):
                    if not check_elements_in_array(wfdd_anomaly_detail_gpt[cls_name] + anomaly_status_general,
                                                   pred_phrases[i]):
                        pred_phrases[i] += "#$%"
                        # boxes_filt_copy = torch.cat([boxes_filt_copy[:i], boxes_filt_copy[i+1:]])
                        continue
    ...
    text = "{} Time for test: {} \n".format(dataset_name, end_time_test - start_time_test)
    print(text)
    with open('/home/dell/wgp3/FiLo/{}_inference_time.txt'.format(dataset_name), 'a', encoding='utf-8') as f:
        f.write(text)
    # metrics
    # table_ls = []
    # auroc_sp_ls = []
    ...
    #     [
    #         "mean",
    #         str(np.round(np.mean(auroc_px_ls) * 100, decimals=4)),
    #         str(np.round(np.mean(f1_px_ls) * 100, decimals=4)),
    #         str(np.round(np.mean(ap_px_ls) * 100, decimals=4)),
    #         str(np.round(np.mean(aupro_ls) * 100, decimals=4)),
    #         str(np.round(np.mean(auroc_sp_ls) * 100, decimals=4)),
    #         str(np.round(np.mean(f1_sp_ls) * 100, decimals=4)),
    #         str(np.round(np.mean(ap_sp_ls) * 100, decimals=4)),
    #     ]
    # )
    # results = tabulate(
    #     table_ls,
