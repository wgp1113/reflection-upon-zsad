BASE_PATH = "/home/dell/wgp/datasets/zsad_datasets"
DATA_PATH = {
    "Brain": f"{BASE_PATH}/data/MedAD/Brain_AD",
    "Liver": f"{BASE_PATH}/data/MedAD/Liver_AD",
    "Retina": f"{BASE_PATH}/data/MedAD/Retina_RESC_AD",
    "Colon_clinicDB": f"{BASE_PATH}/data/Colon/CVC-ClinicDB",
    "Colon_colonDB": f"{BASE_PATH}/data/Colon/CVC-ColonDB",
    "Colon_cvc300": f"{BASE_PATH}/data/Colon/CVC-300",
    "Colon_Kvasir": f"{BASE_PATH}/data/Colon/Kvasir",
    "BTAD": f"{BASE_PATH}/data/BTech_Dataset_transformed",
    "MPDD": f"{BASE_PATH}/data/MPDD",
    "dtd":"",
    "ksdd2":"",
    "wfdd":"",
    "MVTec": "",
    "VisA": "",
    # "MVTec": f"{BASE_PATH}/mvtec_zsad_common_sense",
    # "VisA": f"{BASE_PATH}/VisA_zsad_common_sense",
    # "MVTec": f"{BASE_PATH}/mvtec_zsad_extended",
    # "VisA": f"{BASE_PATH}/VisA_zsad_extended",
}

CLASS_NAMES = {
    "br35h": ["brain"],
    "headct": ["headct"],
    "brainmri": ["brain_mri"],
    "cvc300": ["colon"],
    "clinicdb": ["colon"],
    "colondb": ["colon"],
    "isic": ["skin_cancer"],
    "Brain": ["Brain"],
    "Liver": ["Liver"],
    "Retina": ["Retina"],
    "Colon_clinicDB": ["Colon_clinicDB"],
    "Colon_colonDB": ["Colon_colonDB"],
    "Colon_Kvasir": ["Kvasir"],
    "Colon_cvc300": ["CVC-300"],
    "dtd":['Woven_001', 'Woven_127', 'Woven_104', 'Stratified_154', 'Blotchy_099', 'Woven_068', 'Woven_125', 'Marbled_078', 'Perforated_037', 'Mesh_114', 'Fibrous_183', 'Matted_069'],
    "ksdd2":['surface'],
    "wfdd":['grey_cloth', 'grid_cloth', 'pink_flower', 'yellow_cloth'],
    "MVTec": [
        # "bottle",
        "cable",
        "capsule",
        "carpet",
        "grid",
        "hazelnut",
        "leather",
        # "metal_nut",
        # "pill",
        "screw",
        "tile",
        "transistor",
        "toothbrush",
        "wood",
        "zipper",
    ],
    "VisA": [
        "candle",
        "pcb3",
        "capsules",
        "pipe_fryum",
        # "pcb4",
        "macaroni2",
        "pcb2",
        "chewinggum",
        "macaroni1",
        "cashew",
        "fryum",
        "pcb1",
    ],
    "MPDD": [
        "connector",
        "tubes",
        "metal_plate",
        "bracket_white",
        "bracket_brown",
        "bracket_black",
    ],
    "BTAD": ["01", "02", "03"],
}
DOMAINS = {
    "br35h": "Medical",
    "headct": "Medical",
    "brainmri": "Medical",
    "cvc300": "Medical",
    "clinicdb": "Medical",
    "colondb": "Medical",
    "isic": "Medical",
    "VisA": "Industrial",
    "BTAD": "Industrial",
    "MPDD": "Industrial",
    "MVTec": "Industrial",
    "Brain": "Medical",
    "dtd": "Industrial",
    "ksdd2": "Industrial",
    "wfdd": "Industrial",
    "Liver": "Medical",
    "Retina": "Medical",
    "Colon_clinicDB": "Medical",
    "Colon_colonDB": "Medical",
    "Colon_Kvasir": "Medical",
    "Colon_cvc300": "Medical",
}
REAL_NAMES = {
    "br35h": {"brain":"scan"},
    "headct": {"headct":"scan"},
    "brainmri": {"brain_mri":"scan"},
    "cvc300": {"colon":"colon endoscopy image"},
    "clinicdb": {"colon":"colon endoscopy image"},
    "colondb": {"colon":"colon endoscopy image"},
    "isic": {"skin_cancer":"skin cancer image"},
    "Brain": {"Brain": "scan"},
    "Liver": {"Liver": "scan"},
    "Retina": {"Retina": "scan"},
    "MVTec": {
        # "bottle": "dark bottle",
        "cable": "top view of three cables",
        "capsule": "black and orange capsule",
        "carpet": "gray carpet",
        "grid": "metal or plastic mesh",
        "hazelnut": "single brown hazelnut",
        "leather": "brown leather",
        # "metal_nut": "metal nut which has four notched edges",
        # "pill": "oval white pill with small red speckles and the letters 'FF' engraved",
        "screw": "screw",
        "tile": "speckled tile surface",
        "transistor": "a three-legged transistor placed vertically",
        "toothbrush": "toothbrush head",
        "wood": "wood surface",
        "zipper": "a black zipper",
    },
    "VisA": {
        "candle": "candle",
        "pcb3": "infrared sensor pcb module",
        "capsules": "capsules",
        "pipe_fryum": "pipe-shaped fryum",
        # "pcb4": "battery charging pcb module",
        "macaroni2": "scattered yellow macaroni",
        "pcb2": "integrated circuits board",
        "chewinggum": "chewing gum",
        "macaroni1": "orange macaroni",
        "cashew": "cashew nut",
        "fryum": "wheel-shaped fryum snack",
        "pcb1": "dual ultrasonic distance sensor pcb module",
    },
    "dtd":{
        'Woven_001':"interwoven metallic textured strips pattern",
        'Woven_127':"industrial silver and gold metallic wire mesh",
        'Woven_104':"coarse brown burlap weave texture",
        'Stratified_154':"reddish-brown fine-grained wood texture",
        'Blotchy_099':"grey mottled stone texture",
        'Woven_068':"natural golden wicker weave texture",
        'Woven_125':"blue denim fabric weave texture",
        'Marbled_078':"dark grey marble stone texture",
        'Perforated_037':"perforated silver metal sheet texture",
        'Mesh_114':"coiled silver metallic wire mesh",
        'Fibrous_183':"intertwined yellow wood fiber texture",
        'Matted_069':"tangled dry grass and fiber texture",},
    "ksdd2":{'surface':"dark charcoal grainy felt texture"},
    "wfdd":{
        'grey_cloth':"woven grey textile fabric",
        'grid_cloth':"black and white checkered plaid fabric",
        'pink_flower':"floral patterned pink fabric with green leaves",
        'yellow_cloth':"woven yellow cotton fabric with texture"},

    "Colon_clinicDB": {
        "Colon_clinicDB": "colon endoscopy image",
    },
    "Colon_colonDB": {
        "Colon_colonDB": "colon endoscopy image",
    },
    "Colon_cvc300": {"CVC-300": "colon endoscopy image"},
    "Colon_Kvasir": {"Kvasir": "colon endoscopy image"},
    "MPDD": {
        "connector": "metal clamps with black adjustment knobs",
        "tubes": "scattered metal objects",
        "metal_plate": "blue rectangular metal plate with a notch on one side",
        "bracket_white": "white, elongated triangular metal bracket with a smooth, matte finish",
        "bracket_brown": "brown L-shaped metal bracket with smooth, glossy finish and multiple mounting holes along its arms",
        "bracket_black": "black ornamental metal bracket with spiral design attached to a rectangular frame",
    },
    "BTAD": {
        "01": "Bright concentric rings in neon yellow and blue tones against a dark blue background, resembling a stylized wave or energy field radiating outward.",
        "02": "vertical fabric lines in warm, dusty pink and beige tones",
        "03": "oval concentric circular rings in gradient shades of blue and white",
    },
}
PROMPTS = {
    "prompt_normal": ["{}", "a {}", "the {}"],
    "prompt_abnormal": [
        "a damaged {}",
        "a broken {}",
        "a {} with flaw",
        "a {} with defect",
        "a {} with damage",
    ],
    "prompt_templates": [
        "{}.",
        "a photo of {}.",
    ],
}