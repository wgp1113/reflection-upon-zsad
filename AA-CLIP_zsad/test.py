...
def get_predictions(
    ...
    for input_data in tqdm(test_loader):
        ...
        label = input_data["anomaly"].cpu().numpy()
        file_name = input_data["img_path"]
        # set up class-specific containers
        class_name = input_data["cls_name"]
...
def main():
    ...
    parser.add_argument("--checkpoint_path", type=str, default="ckpt/baseline")
    ...
    text_file = glob(args.checkpoint_path + "/text_adapter.pth")
    ...
    # files = sorted(glob(args.checkpoint_path + "/image_adapter_*.pth"))
    files = sorted(glob(args.checkpoint_path + "/image_adapter_20.pth"))
    ...
    for file in files:
        ...
        # df.loc[len(df),'P-AUROC':'I-AP'] = df.loc[:,'P-AUROC':'I-AP'].mean()
        # df.loc[len(df) - 1, "class name"] = "Average"
        # logger.info("final results:\n%s", df.to_string(index=False, justify="center"))
    text = "Time for testing mvtec: {}s \n".format(total_time)
    print(text)
    with open('/home/dell/wgp3/AA-CLIP_zsad/mvtec_inference_time.txt', 'a', encoding='utf-8') as f:
        f.write(text)
