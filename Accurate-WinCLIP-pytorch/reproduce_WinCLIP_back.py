...
def test(args,):
    ...
    if dataset_name == 'mvtec':
        obj_list = ['carpet', 'hazelnut', 'leather', 'cable', 'capsule', 'grid',
        ...
        obj_list = ['Woven_001', 'Woven_127', 'Woven_104', 'Stratified_154',
        ...
        obj_list = ['grey_cloth', 'grid_cloth', 'pink_flower', 'yellow_cloth']
        test_data = OtherDataset(dataset_name, root=dataset_dir, transform=preprocess, target_transform=transform,
                                 mode='test')
    ...
    with open('/home/dell/wgp3/Accurate-WinCLIP-pytorch/mvtec_inference_time.txt', 'a', encoding='utf-8') as f:
        f.write(text)
...
#visualization  image_path_list, pr_px, category
    ...
    #
    #
    # table_ls.append(['mean', str(np.round(np.mean(auroc_px_ls) * 100, decimals=4)),
    #                  str(np.round(np.mean(f1_px_ls) * 100, decimals=4)), str(np.round(np.mean(ap_px_ls) * 100, decimals=4)),
    #                  str(np.round(np.mean(aupro_ls) * 100, decimals=4)), str(np.round(np.mean(auroc_sp_ls) * 100, decimals=4)),
    #                  str(np.round(np.mean(f1_sp_ls) * 100, decimals=4)), str(np.round(np.mean(ap_sp_ls) * 100, decimals=4))])
