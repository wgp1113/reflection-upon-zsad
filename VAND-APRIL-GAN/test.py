...
from dataset import VisaDataset, MVTecDataset , OtherDataset
...
def test(args):
    ...
    if dataset_name == 'mvtec':
        ...
        test_data = OtherDataset(root=dataset_dir, transform=preprocess, target_transform=transform, mode='test')
    ...
    text = "Time for training: {}s \n".format(end_time - start_time)
    with open('/mnt/data3/wgp3/VAND-APRIL-GAN/mvtec_inference_time.txt', 'a', encoding='utf-8') as f:
        f.write(text)
            # # visualization
            # path = items['img_path']
            # cls = path[0].split('/')[-2]
